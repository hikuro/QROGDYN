################################ zeroPtraj.py ###################################
#
#   This program makes random phase zoro-point vibration trajectory
#   according to harmonic oscillator approximation.
#   You must prepare freqinHP, progdyn.conf, solvgeo.pdb.
#
#   ver 04/16/19 written by Hiroaki Kurouchi
#
################################################################################
...
def centeratomPhaseMake(origdir,freqfile):
    ...
    amplitude = 1e10 * (h/(2*np.pi*angular_velocity * 1E15 * redMass * 1.6605402E-27))**0.5  #unit:angstrom
    ...
    with open(origdir + "zerotraj",mode="w") as cw:
        for i in range(1000):
            cw.write(str(len(rotated_mode[0]))+"\n0.0 Comment 1 2 3 4 "+str(i)+"\n")
            geoArrZero = geoArr + np.sum((amplitude * rotated_mode.T * np.sin(phase +  i * angular_velocity)).T,axis=0)
            for j in range(len(geoArrZero)):
                cw.write(str(atSym[j])+" {: .7f} {: .7f} {: .7f}\n".format(geoArrZero[j][0],geoArrZero[j][1],geoArrZero[j][2]))
...
if  __name__ == "__main__":
    ...
    if origdir[-1] != "/":
        origdir = origdir + "/"
    centeratomfix = 2 #int(progread.confread_each(origdir,"centeratomfix",0)[1])
    if os.path.exists(origdir+pdbfile) == True:
